...
@DatasetReader.register('good_news_ner')
class GoodNewsNERReader(DatasetReader):
    ...
    def _read(self, split: str):
        ...
        dataset_size = len(raw_str_captions)
        ...
        for i in tqdm(ids):
            ...
            article = raw_str_articles[i]
            caption = raw_str_captions[i]
            article_url = article_urls[i]

            if not article or len(article) == 0:
                continue
                
            article = ' '.join(article.strip().split(' ')[:500])
