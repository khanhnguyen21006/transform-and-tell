...
@TokenIndexer.register("roberta_gns_ner")
class RobertaGNsNERTokenIndexer(TokenIndexer[int]):
    def __init__(self,
    ...
    def tokens_to_indices(self,
                          ...
                          index_name: str,
                          caption: bool = False) -> Dict[str, List[int]]:
        if not self._added_to_vocabulary:
            self._add_encoding_to_vocabulary(vocabulary)
            self._added_to_vocabulary = True

        text = ' '.join([token.text for token in tokens])
        indices, copy_masks, noise_mask, ner_mask = self.encode(text, caption) # modify
    ...
    def encode(self, sentence, caption=False):
        if self.legacy:
            return self.encode_legacy(sentence)
            
        bpe_tokens, copy_masks, noise_mask, ner_mask = self._byte_pair_encode(sentence, caption) # modify
    ...
    def _byte_pair_encode(self, text, caption=False):
        bpe_raw_tokens = []
        bpe_tokens = []
        bpe_copy_masks = []
        ...
        copy_masks = self.get_entity_mask(raw_tokens, None)
        ...
        if caption:
            ...
            ner_masks = [False] * len(raw_tokens)
        ...
        for ent in doc.ents:
            if random.random() < 0.8:
                ...
                for i, (start, end, token) in enumerate(zip(starts, ends, tokens)):
                    ...
                    if start >= entity_start and end <= entity_end and ent.label_ in ['FAC', 'EVENT', 'PERSON', 'ORG', 'GPE']:
                        copy_masks[i] = 1
        return copy_masks
